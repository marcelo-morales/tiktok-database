...
WITH maxVideo AS (
    SELECT MAX(Revenue.amount) AS 'highest', Video.videoId AS 'maxVideo'
    FROM Revenue JOIN Video ON Revenue.audioID = Video.videoId
)
SELECT Video.comments
FROM Video, maxVideo
WHERE Video.comments = maxVideo.highest AND Video.videoId = maxVideo.maxVideo;
